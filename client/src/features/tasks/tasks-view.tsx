...
export function TasksView({ admin = false }: { admin?: boolean }) {
  ...
  return (
    <div className="desk-grid-glow space-y-6">
      ...
      <section
        className="desk-toolbar flex-col items-stretch gap-0 overflow-hidden p-0"
        aria-label={t.filters}
      >
        <div className="flex flex-col gap-3 border-b border-[var(--border)]/70 p-3 sm:flex-row sm:items-center sm:p-4">
          <label className="relative min-w-0 flex-1">
            <Search className="pointer-events-none absolute top-1/2 left-3.5 size-4 -translate-y-1/2 text-[var(--primary)]" />
            <Input
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t.searchPlaceholder}
              className="border-transparent bg-[var(--surface-muted)] pl-10 focus:bg-[var(--surface)]"
              aria-label={t.searchLabel}
            />
          </label>
          <div className="flex items-center justify-between gap-2 sm:justify-end">
            <span className="hidden items-center gap-2 rounded-full px-3 py-2 text-xs font-black tracking-wide text-[var(--muted)] uppercase md:inline-flex">
              <SlidersHorizontal className="size-3.5 text-[var(--primary)]" />
              {t.filters}
            </span>
            <Button
              variant="ghost"
              ...
              onClick={() => {
                ...
                setSortBy("createdAt");
                setPage(1);
                router.replace(admin ? "/admin/tickets" : "/tickets", { scroll: false });
              }}
        ...
        <div className="grid gap-2 bg-[var(--surface-muted)]/45 p-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 sm:p-4">
          <Select
            value={status}
            onChange={(event) => {
          ...
          <Select
            value={source}
            onChange={(event) => {
              setSource(event.target.value as TicketSource | "");
              setPage(1);
            }}
          ...
          >
            ...
            <option value="resolutionDueAt">{t.slaSoon}</option>
            <option value="firstResponseDueAt">{t.responseSoon}</option>
            <option value="dueDate">{t.requestedSoon}</option>
          ...
          {admin && (
            <Select
              value={attention}
              onChange={(event) => {
                setAttention(event.target.value as (typeof ticketAttention)[number] | "");
                setPage(1);
              }}
              aria-label={t.attentionFilter}
            >
              <option value="">{t.allAttention}</option>
              <option value="first-response-breached">{t.firstResponseBreached}</option>
              <option value="resolution-breached">{t.resolutionBreached}</option>
              <option value="requested-overdue">{t.requestedOverdue}</option>
            </Select>
          )}
        </div>
      </section>
      ...
      {pagination && pagination.totalPages > 1 && (
        <nav
          className="desk-panel flex flex-wrap items-center justify-between gap-3 p-3 text-sm sm:p-4"
          aria-label={`${t.page} ${formatNumber(pagination.page, intlLocale)}`}
        >
          <span className="flex items-center gap-2 text-[var(--muted)]">
            <span className="desk-icon-well size-8">
