    ...
    <>
      <div className="md:flex md:h-[calc(100dvh-9.25rem)] md:min-h-0 md:flex-col md:overflow-hidden">
        <header className="desk-page-header relative overflow-hidden rounded-[var(--container-radius)] border bg-[color-mix(in_srgb,var(--surface)_92%,transparent)] px-5 py-5 shadow-sm sm:px-6">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid grid-cols-3 gap-2 sm:min-w-[22rem]">
              {[
                { label: t.visible, value: queueCounts.visible, icon: Inbox },
                { label: t.waitingCount, value: queueCounts.waiting, icon: Clock3 },
                { label: t.activeCount, value: queueCounts.active, icon: Headphones },
        ...
        {chatsQuery.isPending ? (
          ...
          <div
            className={cn(
              "desk-panel desk-grid-glow mt-4 grid min-h-[38rem] overflow-hidden rounded-[var(--container-radius)] border bg-[var(--surface)] shadow-[var(--shadow-panel)] md:min-h-0 md:flex-1 xl:grid-cols-[21rem_minmax(0,1fr)] 2xl:grid-cols-[21rem_minmax(0,1fr)_17rem]",
              threadOpen &&
          ...
          >
            ...
            >
              <div className="desk-toolbar flex min-h-18 shrink-0 items-center justify-between gap-3 border-b px-4 py-3">
                <div>
                  ...
                  <p className="mt-0.5 text-[0.625rem] font-medium text-[var(--muted)]">
                    {queueCounts.waiting} {t.waitingCount.toLowerCase()} ·{" "}
                    {queueCounts.active} {t.activeCount.toLowerCase()}
                  </p>
                </div>
                <span className="rounded-full border bg-[var(--surface)] px-2.5 py-1 text-[0.625rem] font-bold text-[var(--primary)] tabular-nums">
                  {chatsQuery.data?.pagination.total ?? chats.length}
                </span>
              </div>
              ...
              {!selected ? (
                ...
                <>
                  ...
                  {selected.linkedTicket && (
                    <Link
                      href={`/admin/tickets?search=${encodeURIComponent(selected.linkedTicket.ticketNumber || selected.linkedTicket.id)}`}
                      className="focus-ring group flex min-h-14 shrink-0 flex-wrap items-center gap-2.5 border-b bg-[color-mix(in_srgb,var(--primary-soft)_44%,var(--surface))] px-4 py-2.5 text-xs transition-colors hover:bg-[var(--primary-soft)]"
                      aria-label={`${t.linkedTicket}: ${selected.linkedTicket.ticketNumber}`}
                    >
                      <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-[var(--surface)] text-[var(--primary)] shadow-sm">
                        <TicketCheck className="size-4" aria-hidden="true" />
                      </span>
                      <span className="font-mono font-bold text-[var(--primary)]">
                        {selected.linkedTicket.ticketNumber}
                      </span>
                      <span className="min-w-32 flex-1 truncate font-semibold">
                        {selected.linkedTicket.title}
                      </span>
                      <TaskPriorityBadge priority={selected.linkedTicket.priority}>
                        {getTaskPriorityLabel(selected.linkedTicket.priority, locale)}
                      </TaskPriorityBadge>
                      <TaskStatusBadge status={selected.linkedTicket.status}>
                        {getTaskStatusLabel(selected.linkedTicket.status, locale)}
                      </TaskStatusBadge>
                  ...
                  <footer className="shrink-0 border-t bg-[color-mix(in_srgb,var(--surface)_94%,transparent)] p-3 shadow-[0_-12px_32px_rgba(15,23,42,.04)] backdrop-blur-xl max-md:pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-4">
                    ...
                    {!canReply && selected.status !== "ended" && (
                      <p className="mb-2 flex items-center gap-2 rounded-xl bg-[var(--surface-muted)] px-3 py-2 text-[0.6875rem] font-medium text-[var(--muted)]">
